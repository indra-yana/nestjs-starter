import { AuthService, GoogleAuthResponse } from 'src/core/common/auth/auth.service';
...
export class GoogleAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        try {
            ...
            const userInfo = await this.google.request<GoogleAuthResponse>({
                url: 'https://www.googleapis.com/oauth2/v3/userinfo',
            }); 
          
            const user: GoogleAuthResponse = userInfo?.data;
            if (!user) {
            ...
            request['user'] = await this.authService.socialAuth(user, SOCIAL_AUTH.GOOGLE);
